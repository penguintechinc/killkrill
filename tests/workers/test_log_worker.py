...
class TestElasticsearchProcessor:
    ...
    def test_process_logs_batch_success(self, mock_config, mock_es, sample_log_batch,
                                       elasticsearch_bulk_response):
        ...
        with patch('elasticsearch.helpers.bulk', return_value=elasticsearch_bulk_response(10, 0)):
    ...
    @patch('apps.log-worker.app.es_client')
    @patch('apps.log-worker.app.config')
    def test_process_logs_batch_partial_failure(self, mock_config, mock_es, sample_log_batch,
                                                 elasticsearch_bulk_response):
        ...
        with patch('elasticsearch.helpers.bulk', return_value=elasticsearch_bulk_response(7, 3)):
            processor = ElasticsearchProcessor()
            result = processor.process_logs_batch(sample_log_batch)
    ...
    def test_convert_to_ecs_document(self, mock_config, sample_log_message):
        ...
        fields_decoded = {k.decode(): v.decode() if isinstance(v, bytes) else v
...
class TestRedisStreamsConsumer:
    ...
    @patch('apps.log-worker.app.config')
    @patch('time.sleep')
    def test_consume_messages_with_data(self, mock_sleep, mock_config, mock_es,
                                       mock_redis, redis_stream_messages):
        ...
        mock_redis.xinfo_groups.return_value = [{'name': 'test-group', 'last-delivered-id': '0-0'}]
    ...
    @patch('apps.log-worker.app.redis_client')
    @patch('apps.log-worker.app.config')
    def test_process_message_batch(self, mock_config, mock_redis, redis_stream_messages):
        ...
        for msg_id, fields in messages:
            fields_bytes = {k.encode(): v.encode() if isinstance(v, str) else v
                          for k, v in fields.items()}
...
class TestWorkerLifecycle:
    ...
    @patch('apps.log-worker.app.config')
    def test_signal_handler_setup(self, mock_config, mock_license, mock_es, mock_redis,
                                  mock_signal_handler):
    ...
    @patch('apps.log-worker.app.es_client')
    @patch('time.sleep')
    def test_graceful_shutdown(self, mock_sleep, mock_es, mock_redis,
                              mock_license, mock_pool):
...
class TestErrorHandling:
    ...
    @patch('apps.log-worker.app.es_client')
    @patch('apps.log-worker.app.config')
    def test_elasticsearch_connection_failure(self, mock_config, mock_es, sample_log_batch):
        ...
        with patch('elasticsearch.helpers.bulk', side_effect=Exception('Connection failed')):
            processor = ElasticsearchProcessor()
            result = processor.process_logs_batch(sample_log_batch)
    ...
    def test_malformed_log_message_handling(self, mock_config):
        ...
        # Malformed message
        malformed = [
            (b'msg-1', {b'invalid_field': b'value'}),
            (b'msg-2', None)
        ]
...
class TestPrometheusMetrics:
    ...
    @patch('apps.log-worker.app.es_client')
    @patch('apps.log-worker.app.config')
    def test_processing_metrics_incremented(self, mock_config, mock_es, sample_log_batch):
