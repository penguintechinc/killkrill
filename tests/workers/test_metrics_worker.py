...
class TestPrometheusDestination:
    ...
    def test_push_metrics_failure(self, mock_post):
        ...
        dest = PrometheusDestination('http://localhost:9091')
        dest.metrics_buffer = [
            {'name': 'test', 'type': 'gauge', 'value': 1.0, 'labels': {}, 'help': 'Test'}
        ]
...
class TestMetricsWorker:
    ...
    @patch('apps.metrics_worker.app.redis_client')
    @patch('apps.metrics_worker.app.config')
    def test_consume_messages_with_data(self, mock_config, mock_redis, sample_metric_batch):
...
class TestErrorHandling:
    ...
    def test_redis_connection_error(self, mock_config, mock_redis):
        ...
        mock_redis.xreadgroup.side_effect = redis.exceptions.ConnectionError('Connection lost')
    ...
    def test_destination_failure_handling(self, mock_config, mock_redis):
        ...
        with patch.object(worker.destinations['prometheus'], 'add_metric', return_value=False):
            metric_data = {
                'name': 'test',
                'type': 'gauge',
                'value': '42.0',
...
class TestBackpressure:
    ...
    def test_slow_destination_handling(self, mock_config, mock_redis):
        ...
        with patch.object(worker.destinations['prometheus'], 'add_metric',
                         side_effect=slow_add_metric):
            metric_data = {
                'name': 'test',
                'type': 'gauge',
                'value': '42.0',
