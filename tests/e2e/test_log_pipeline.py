...
def generate_unique_log(level: str = 'info', source: str = 'e2e-test') -> Dict[str, Any]:
...
def wait_for_redis_log(redis_client, test_id: str, timeout: int = PROCESSING_TIMEOUT) -> bool:
            ...
            for stream_name in ['logs', 'logs:raw']:
                try:
                    ...
                    for msg_id, fields in messages:
                        ...
                        if test_id in message_content or test_id in fields.get('test_id', ''):
                            return True
...
def test_different_log_levels(log_receiver_available, redis_client):
    ...
    assert set(found_levels) == set(levels), \
...
def test_log_worker_processing(log_receiver_available, redis_client):
    ...
    assert wait_for_redis_log(redis_client, test_id), \
        "Log not found in Redis stream"
...
def test_complete_log_pipeline(log_receiver_available, api_available, redis_client):
    ...
    if log_from_api:
        ...
        assert log_data['log_level'] in log_from_api.get('level', '').lower() or \
               log_data['log_level'] in log_from_api.get('log_level', '').lower(), \
               "Log level mismatch"

        assert test_id in log_from_api.get('message', ''), \
               "Test ID not found in message"

        # Verify timestamp preservation
        assert 'timestamp' in log_from_api, "Timestamp missing from API response"
    else:
        pytest.skip("API log query endpoint not yet implemented or log not yet processed")
        ...
        try:
            for stream_name in ['logs', 'logs:raw']:
                try:
                    ...
                    for msg_id, fields in messages:
                        if test_id in fields.get('message', '') or \
                           test_id in fields.get('test_id', ''):
                            ...
                            redis_level = fields.get('level', fields.get('log_level', ''))
                            assert redis_level.lower() == log_data['log_level'].lower(), \
