  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === "prometheus") {
        ...
        const statusRes = await fetch(
          `${API_BASE}/infrastructure/alertmanager/status`,
        );
        const statusData = await statusRes.json();
        setAlertmanagerStatus(statusData);

        const alertsRes = await fetch(
          `${API_BASE}/infrastructure/alertmanager/alerts`,
        );
        const alertsData = await alertsRes.json();
        setAlerts(alertsData.alerts || []);

        const silencesRes = await fetch(
          `${API_BASE}/infrastructure/alertmanager/silences`,
        );
  ...
  const executeElasticsearchQuery = async (e) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/infrastructure/elasticsearch/search`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        <>
          ...
          {activeTab === "prometheus" && (
            <div>
              ...
              {prometheusStatus && (
                <div style={cardStyle}>
                  ...
                  >
                    <StatItem
                      label="Status"
                      value={prometheusStatus.status || "unknown"}
                    />
                    <StatItem
                      label="Version"
                      value={prometheusStatus.version || "N/A"}
                    />
                    <StatItem
                      label="Uptime"
                      value={prometheusStatus.uptime || "N/A"}
                    />
                    <StatItem
                      label="Storage"
                      value={prometheusStatus.storage || "N/A"}
                    />
                  </div>
                </div>
              )}
              ...
              <div style={{ ...cardStyle, marginTop: "1.5rem" }}>
                ...
                <form
                  onSubmit={executePrometheusQuery}
              ...
              <div style={{ ...cardStyle, marginTop: "1.5rem" }}>
                <h2 style={{ color: "#d4af37", marginBottom: "1rem" }}>
                  Targets ({prometheusTargets.length})
                </h2>
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            borderRadius: "4px",
                            backgroundColor:
                              target.health === "up" ? "#2a4a2a" : "#4a2a2a",
                            color:
                              target.health === "up" ? "#6fdc6f" : "#ff6b6b",
                            fontSize: "0.85rem",
                          }}
                        >
          ...
          {activeTab === "elasticsearch" && (
            <div>
                ...
                <div style={cardStyle}>
                  ...
                  >
                    <StatItem
                      ...
                      value={elasticsearchStatus.cluster_health || "unknown"}
                    />
                    <StatItem
                      label="Nodes"
                      value={elasticsearchStatus.nodes || 0}
                    />
                    <StatItem
                      label="Indices"
                      value={elasticsearchStatus.indices || 0}
                    />
                    <StatItem
                      label="Documents"
                      value={elasticsearchStatus.documents || 0}
                    />
                  </div>
                </div>
              )}
              ...
              <div style={{ ...cardStyle, marginTop: "1.5rem" }}>
                ...
                <form
                  onSubmit={executeElasticsearchQuery}
                  style={{ display: "grid", gap: "1rem" }}
                >
                  <textarea
              ...
              <div style={{ ...cardStyle, marginTop: "1.5rem" }}>
                <h2 style={{ color: "#d4af37", marginBottom: "1rem" }}>
                  Indices ({elasticsearchIndices.length})
                </h2>
                <div style={{ display: "grid", gap: "1rem" }}>
          ...
          {activeTab === "grafana" && (
            <div>
              ...
              {grafanaStatus && (
                <div style={cardStyle}>
                  ...
                  >
                    <StatItem
                      label="Status"
                      value={grafanaStatus.status || "unknown"}
                    />
                    <StatItem
                      label="Version"
                      value={grafanaStatus.version || "N/A"}
                    />
                    <StatItem
                      label="Dashboards"
                      value={grafanaStatus.dashboards || 0}
                    />
              ...
              <div style={{ ...cardStyle, marginTop: "1.5rem" }}>
                <h2 style={{ color: "#d4af37", marginBottom: "1rem" }}>
                  Dashboards ({grafanaDashboards.length})
                </h2>
                <div style={{ display: "grid", gap: "1rem" }}>
              ...
              {alertmanagerStatus && (
                <div style={cardStyle}>
                  ...
                  >
                    <StatItem
                      label="Status"
                      value={alertmanagerStatus.status || "unknown"}
                    />
                    <StatItem
                      label="Version"
                      value={alertmanagerStatus.version || "N/A"}
                    />
                    <StatItem
                      label="Active Alerts"
                      value={alertmanagerStatus.active_alerts || 0}
                    />
                    <StatItem
                      label="Silences"
                      value={alertmanagerStatus.silences || 0}
                    />
                  </div>
                </div>
              )}
                ...
                <div style={{ display: "grid", gap: "1rem" }}>
                  {silences.map((silence, idx) => (
                    ...
                    >
                      ...
                      <p style={{ color: "#999", fontSize: "0.9rem" }}>
                        Created by: {silence.createdBy} | Expires:{" "}
                        {silence.endsAt}
                      </p>
                      {silence.matchers && (
                        <div
