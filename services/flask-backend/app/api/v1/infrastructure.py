...
def proxy_request(service: str, path: str, method: str = 'GET', **kwargs) -> Dict[str, Any]:
...
def prometheus_range_query():
    ...
    data, status = proxy_request('prometheus', '/api/v1/query_range', 'GET', params=params)
    return jsonify(data), status
...
def elasticsearch_search():
    ...
    data, status = proxy_request('elasticsearch',
                                f'/{index}/_search',
                                'POST',
...
def alertmanager_create_silence():
    ...
    payload = request.get_json() or {}
    headers = {'Content-Type': 'application/json'}
    data, status = proxy_request('alertmanager',
                                '/api/v1/silences',
                                'POST',
                                json=payload,
                                headers=headers)
