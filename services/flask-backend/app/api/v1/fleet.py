...
def fleet_status():
    ...
    if error:
        return (
            jsonify(ErrorResponse(error=error, code="FLEET_ERROR").model_dump()),
            status_code,
        )
...
def get_host(host_id):
    ...
    if status_code == 404:
        return (
            jsonify(
                ErrorResponse(error="Host not found", code="NOT_FOUND").model_dump()
            ),
            404,
        )
...
def run_query():
    ...
    try:
        ...
        return (
            jsonify(ErrorResponse(error=str(e), code="INTERNAL_ERROR").model_dump()),
            500,
        )
...
def remove_host(host_id):
    ...
    try:
        ...
        return (
            jsonify(
                APIResponse(
                    success=True, data={"message": "Host removed successfully"}
                ).model_dump()
            ),
            200,
        )
