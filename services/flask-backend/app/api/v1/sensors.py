...
def agent_detail(id):
    ...
    if not agent:
        return jsonify(ErrorResponse(error='Agent not found', code='NOT_FOUND').model_dump()), 404

    if request.method == 'GET':
        return jsonify(APIResponse(success=True, data=agent.as_dict()).model_dump()), 200
...
def agent_heartbeat(id):
    ...
    return jsonify(APIResponse(success=True, message='Heartbeat recorded').model_dump()), 200
...
def check_detail(id):
    ...
    if not check:
        return jsonify(ErrorResponse(error='Check not found', code='NOT_FOUND').model_dump()), 404

    if request.method == 'GET':
        return jsonify(APIResponse(success=True, data=check.as_dict()).model_dump()), 200
    ...
    if request.method == 'PUT':
        try:
            ...
            return jsonify(APIResponse(success=True, data=updated_check).model_dump()), 200
        except ValidationError as e:
            return jsonify(ErrorResponse(error=str(e), code='VALIDATION_ERROR').model_dump()), 400
...
def results_list_submit():
    ...
    if request.method == 'GET':
        ...
        results = db(db.sensor_results).select(limitby=(0, limit), orderby=~db.sensor_results.created_at).as_list()
