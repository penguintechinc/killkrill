...
class AIProviderCreate(BaseModel):
    ...
    provider_type: Literal["ollama", "openai", "claude"] = Field(
        description="Provider type"
    )
    endpoint_url: str = Field(
        min_length=1, max_length=512, description="API endpoint URL"
    )
    api_key: Optional[str] = Field(
        default=None, max_length=256, description="API key (required for openai/claude)"
    )
    model: str = Field(
        default="tinyllama", max_length=128, description="Default model to use"
    )
...
class AIChatRequest(BaseModel):
    ...
    provider_id: Optional[int] = Field(
        default=None, description="Provider ID (uses default if not specified)"
    )
    model: Optional[str] = Field(
        default=None, max_length=128, description="Model override"
    )
    max_tokens: int = Field(
        default=1024, ge=1, le=32000, description="Max tokens in response"
    )
    temperature: float = Field(
        default=0.7, ge=0.0, le=2.0, description="Sampling temperature"
    )
    system_prompt: Optional[str] = Field(
        default=None, max_length=4000, description="System prompt"
    )
...
class AIAnalyzeRequest(BaseModel):
    ...
    data_source: str = Field(
        min_length=1, max_length=256, description="Data source identifier"
    )
    analysis_type: Literal[
        "general", "anomaly_detection", "correlation", "forecasting"
    ] = "general"
    input_data: dict = Field(
        default_factory=dict, description="Input data for analysis"
    )
...
def is_free_tier_request(provider_type: str, model: str) -> bool:
    ...
    return (
        provider_type.lower() == FREE_TIER_PROVIDER and model.lower() == FREE_TIER_MODEL
    )
...
def get_default_provider():
    ...
    db = get_pydal_connection()
    provider = (
        db((db.ai_providers.is_default == True) & (db.ai_providers.is_active == True))
        .select()
        .first()
    )
...
def validate_provider_access(provider_type: str, model: str) -> tuple[bool, str]:
    ...
    if is_free_tier_request(provider_type, model):
        ...
        if not has_valid_license() and get_provider_count() >= FREE_TIER_MAX_ENDPOINTS:
            return (
                False,
                f"Free tier limited to {FREE_TIER_MAX_ENDPOINTS} Ollama endpoint. Upgrade license for more.",
            )
        return True, ""
    ...
    if not has_valid_license():
        if provider_type.lower() in LICENSED_PROVIDERS:
            return (
                False,
                f"{provider_type.title()} requires a KillKrill license. Free tier supports Ollama with tinyllama only.",
            )
        if model.lower() != FREE_TIER_MODEL:
            return (
                False,
                f'Model "{model}" requires a license. Free tier supports tinyllama model only.',
            )
...
async def call_ollama(
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        response = await client.post(
            f"{endpoint_url.rstrip('/')}/api/generate", json=payload
        )
    ...
    try:
        ...
        if not has_valid_license():
            providers = [
                p
                for p in providers
                if p["provider_type"] == FREE_TIER_PROVIDER
                and p["model"] == FREE_TIER_MODEL
            ]
        ...
        for p in providers:
            if p.get("api_key"):
                p["api_key"] = (
                    "***" + p["api_key"][-4:] if len(p["api_key"]) > 4 else "****"
                )
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to list providers", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def create_provider():
    ...
    try:
        ...
        if not allowed:
            return (
                jsonify(
                    ErrorResponse(error=error_msg, code="LICENSE_REQUIRED").model_dump()
                ),
                403,
            )
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to create provider", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def get_provider(id):
    ...
    try:
        ...
        if not has_valid_license():
            if (
                provider.provider_type != FREE_TIER_PROVIDER
                or provider.model != FREE_TIER_MODEL
            ):
                return (
                    jsonify(
                        ErrorResponse(
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to get provider", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def delete_provider(id):
    ...
    try:
        ...
        return (
            jsonify(APIResponse(success=True, message="Provider deleted").model_dump()),
            200,
        )
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to delete provider", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def list_models():
    ...
    try:
                ...
                models.append(
                    {
                        ...
                        "free_tier": is_free_tier_request(
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to list models", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def chat():
    ...
    try:
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Chat request failed", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def analyze():
    ...
    try:
        ...
        # Validate access
        allowed, error_msg = validate_provider_access(
            provider.provider_type, provider.model
        )
        if not allowed:
            return (
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to create analysis", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def results_list():
    ...
    try:
        ...
        results = (
            db(query)
            .select(limitby=(0, limit), orderby=~db.ai_analyses.created_at)
            .as_list()
        )

        return (
            jsonify(APIResponse(success=True, data={"results": results}).model_dump()),
            200,
        )
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to list results", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def result_detail(id):
    ...
    try:
        ...
        return (
            jsonify(APIResponse(success=True, data=analysis.as_dict()).model_dump()),
            200,
        )
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to get result", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def get_insights():
    ...
    try:
        ...
        analyses = (
            db(db.ai_analyses.status == "completed")
            .select(limitby=(0, limit), orderby=~db.ai_analyses.created_at)
            .as_list()
        )
        ...
        return (
            jsonify(
                APIResponse(success=True, data={"insights": insights}).model_dump()
            ),
            200,
        )
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to get insights", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def get_recommendations():
    ...
    try:
        ...
        analyses = (
            db(db.ai_analyses.status == "completed")
            .select(limitby=(0, 100), orderby=~db.ai_analyses.created_at)
            .as_list()
        )
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to get recommendations", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
    ...
    try:
        ...
        if not data.get("metrics") or not isinstance(data.get("metrics"), list):
            return (
                jsonify(
                    ErrorResponse(
                        error="metrics array is required", code="VALIDATION_ERROR"
                    ).model_dump()
                ),
                400,
            )
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to create anomaly detection", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
...
def get_config():
    ...
    try:
        ...
        config = {
            ...
            "supported_providers": (
                SUPPORTED_PROVIDERS if licensed else [FREE_TIER_PROVIDER]
            ),
            "features": {
                "chat": True,
                "analyze": True,
        ...
        return (
            jsonify(
                ErrorResponse(
                    error="Failed to get config", code="SERVER_ERROR"
                ).model_dump()
            ),
            500,
        )
