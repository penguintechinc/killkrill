...
def _get_overview_data() -> Dict[str, Any]:
    ...
    avg_response = db(db.sensor_results.created_at >= yesterday).select(
        db.sensor_results.response_time_ms.avg()
    ).first()
    avg_response_time = float(avg_response[db.sensor_results.response_time_ms.avg()] or 0)

    uptime_pct = 100.0
    if total_results_24h > 0:
        uptime_pct = ((total_results_24h - failed_results_24h) / total_results_24h) * 100
...
def _get_service_details(name: str) -> Optional[Dict[str, Any]]:
    ...
    service.update({
        ...
        'dependencies': ['postgres', 'redis'] if name in ['api', 'manager'] else ['redis'],
    })
    return service
...
def _get_activity_log(limit: int = 50) -> List[Dict[str, Any]]:
    ...
    for result in results:
        event_type = 'check_passed' if result.sensor_results.status == 'up' else 'check_failed'
        ...
        activities.append({
            ...
            'severity': severity,
            'message': result.sensor_results.error_message or f"Check {result.sensor_results.status}",
        })
...
def get_services():
    ...
    logger.info('dashboard_services_retrieved',
                count=len(services),
                correlation_id=g.get('correlation_id'))
...
def get_service_details(service_name: str):
    ...
    if not service:
        logger.warning('service_not_found',
    ...
    logger.info('dashboard_service_details_retrieved',
                service_name=service_name,
                correlation_id=g.get('correlation_id'))
...
def get_metrics():
    ...
    logger.info('dashboard_metrics_retrieved',
                correlation_id=g.get('correlation_id'))
...
def get_activity():
    ...
    logger.info('dashboard_activity_retrieved',
                count=len(activity),
                limit=limit,
                correlation_id=g.get('correlation_id'))
