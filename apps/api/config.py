...
class QuartConfig:
    ...
    DEBUG: bool = field(
        default_factory=lambda: config("DEBUG", default=False, cast=bool)
    )
    TESTING: bool = False
    ...
    DB_POOL_SIZE: int = field(
        default_factory=lambda: config("DB_POOL_SIZE", default=10, cast=int)
    )
    DB_MIGRATE: bool = field(
        default_factory=lambda: config("DB_MIGRATE", default=True, cast=bool)
    )

    # Redis
    REDIS_URL: str = field(
        default_factory=lambda: config("REDIS_URL", default="redis://localhost:6379/0")
    )
    REDIS_MAX_CONNECTIONS: int = field(
        default_factory=lambda: config("REDIS_MAX_CONNECTIONS", default=20, cast=int)
    )
    ...
    PRODUCT_NAME: str = field(
        default_factory=lambda: config("PRODUCT_NAME", default="killkrill")
    )
    ...
    CORS_ORIGINS: str = field(
        default_factory=lambda: config("CORS_ORIGINS", default="*")
    )

    # Rate Limiting
    RATE_LIMIT_DEFAULT: str = field(
        default_factory=lambda: config("RATE_LIMIT_DEFAULT", default="100/minute")
    )
    ...
    KIBANA_URL: str = field(
        default_factory=lambda: config("KIBANA_URL", default="http://kibana:5601")
    )
    GRAFANA_URL: str = field(
        default_factory=lambda: config("GRAFANA_URL", default="http://grafana:3000")
    )
    ...
    FLEET_SERVER_URL: str = field(
        default_factory=lambda: config(
            "FLEET_SERVER_URL", default="http://fleet-server:8080"
        )
    )
    FLEET_API_TOKEN: str = field(
        default_factory=lambda: config("FLEET_API_TOKEN", default="")
    )
    ...
    METRICS_RECEIVER_URL: str = field(
        default_factory=lambda: config(
