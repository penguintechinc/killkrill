...
async def prometheus_status():
    ...
    try:
        async with httpx.AsyncClient() as client:
            # Get runtime info
            runtime = await client.get(f'{config.PROMETHEUS_URL}/api/v1/status/runtimeinfo', timeout=5.0)
            # Get build info
            build = await client.get(f'{config.PROMETHEUS_URL}/api/v1/status/buildinfo', timeout=5.0)

            return jsonify({
                'status': 'healthy',
                'runtime': runtime.json().get('data', {}) if runtime.status_code == 200 else {},
                'build': build.json().get('data', {}) if build.status_code == 200 else {},
            })
...
async def prometheus_targets():
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f'{config.PROMETHEUS_URL}/api/v1/targets', timeout=10.0)
            return jsonify(response.json())
    except Exception as e:
        return jsonify({'error': str(e)}), 503
...
async def elasticsearch_cluster():
    ...
    try:
        async with httpx.AsyncClient() as client:
            health = await client.get(f'{config.ELASTICSEARCH_URL}/_cluster/health', timeout=10.0)
            stats = await client.get(f'{config.ELASTICSEARCH_URL}/_cluster/stats', timeout=10.0)
...
async def elasticsearch_indices():
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            return jsonify({'indices': response.json() if response.status_code == 200 else []})
    except Exception as e:
        return jsonify({'error': str(e)}), 503
...
async def grafana_dashboards():
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            return jsonify({'dashboards': response.json() if response.status_code == 200 else []})
    except Exception as e:
        return jsonify({'error': str(e)}), 503
...
async def alertmanager_status():
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f'{config.ALERTMANAGER_URL}/api/v2/status', timeout=5.0)
            return jsonify(response.json() if response.status_code == 200 else {'status': 'error'})
    except Exception as e:
        return jsonify({'error': str(e)}), 503
...
async def alertmanager_alerts():
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f'{config.ALERTMANAGER_URL}/api/v2/alerts', timeout=10.0)
            return jsonify({'alerts': response.json() if response.status_code == 200 else []})
    except Exception as e:
        return jsonify({'error': str(e)}), 503
