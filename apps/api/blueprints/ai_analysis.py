...
async def list_analyses():
    ...
    analyses = db(db.ai_analyses).select(
        orderby=~db.ai_analyses.timestamp, limitby=(0, limit)
    )
    ...
    return jsonify(
        {
            ...
            "affected_components": (
                json.loads(analysis.affected_components)
                if analysis.affected_components
                else []
            ),
            "metrics_analyzed": (
                json.loads(analysis.metrics_analyzed)
                if analysis.metrics_analyzed
                else {}
            ),
            ...
            "acknowledged_at": (
                analysis.acknowledged_at.isoformat()
                if analysis.acknowledged_at
                else None
            ),
        }
    )
