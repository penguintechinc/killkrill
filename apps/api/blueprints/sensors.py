...
from middleware.auth import require_auth, require_role, generate_api_key, hash_api_key
...
async def get_sensor(agent_id: str):
    ...
    # Get recent results
    results = db(
        db.sensor_results.agent_id == agent_id
    ).select(orderby=~db.sensor_results.timestamp, limitby=(0, 10))
...
async def get_check(check_id: str):
    ...
    results = db(
        db.sensor_results.check_id == check_id
    ).select(orderby=~db.sensor_results.timestamp, limitby=(0, 50))
...
async def update_check(check_id: str):
    ...
    update_fields = {}
    allowed_fields = ['name', 'target', 'port', 'path', 'expected_status',
                      'timeout_ms', 'interval_seconds', 'is_active']
...
async def query_results():
    ...
    if agent_id:
        query = query.agent_id == agent_id if not check_id else query & (db.sensor_results.agent_id == agent_id)
    if status:
        query = query.status == status if not check_id and not agent_id else query & (db.sensor_results.status == status)

    if check_id or agent_id or status:
        results = db(query).select(orderby=~db.sensor_results.timestamp, limitby=(0, limit))
    else:
        results = db(db.sensor_results).select(orderby=~db.sensor_results.timestamp, limitby=(0, limit))
...
async def get_status_summary():
    ...
    for check in checks:
        ...
        latest = db(
            db.sensor_results.check_id == check.check_id
        ).select(orderby=~db.sensor_results.timestamp, limitby=(0, 1)).first()
        ...
        check_status = {
            ...
            'last_check': latest.timestamp.isoformat() if latest and latest.timestamp else None,
