...
def create_proxy_handler(service_name: str, base_url: str):
    ...
    @action(
        f"proxy/{service_name}/<path:path>", method=["GET", "POST", "PUT", "DELETE"]
    )
    @CORS()
    @require_auth()
    def proxy_handler(path="", current_user=None):
        ...
        try:
            ...
            if request.method == "GET":
                resp = requests.get(
                    target_url, headers=headers, timeout=PROXY_TIMEOUT, stream=True
                )
            ...
            if "text/html" in content_type:
                ...
                if "<head>" in content:
                    ...
                    content = content.replace(
                        "<html>", f"<html><head>{base_tag}</head>"
                    )
                else:
            ...
            try:
                ...
                if resp.status_code == 200:
                    health_status[service] = {
                        ...
                        "details": (
                            resp.json()
                            if service in ["grafana", "elasticsearch", "kibana"]
                            else None
                        ),
                    }
                else:
