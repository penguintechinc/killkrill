...
import requests
from py4web import action, redirect, request, response
...
def prometheus_proxy_handler(path=""):
    ...
    try:
        proxy_response = prometheus_proxy.make_request(
            path, method=request.method, params=request.query_string
        )
    ...
    for service_name, service_info in services.items():
        try:
            ...
            health_status[service_name] = {
                "status": (
                    "healthy" if proxy_response.status_code == 200 else "unhealthy"
                ),
                "status_code": proxy_response.status_code,
                "url": service_info["url"],
                "response_time_ms": (
                    proxy_response.elapsed.total_seconds() * 1000
                    if hasattr(proxy_response, "elapsed")
                    else 0
                ),
            }
