#!/usr/bin/env python3
"""
KillKrill Log Receiver - Working py4web Application
Proper py4web app structure to avoid module routing issues
"""
...
from py4web import action, request, response, DAL, Field
...
try:
    ...
    db = DAL(pydal_database_url, migrate=True, fake_migrate=True)

    # Create basic tables (skip if exists)
    try:
        db.define_table('logs',
            ...
            Field('message', 'text'),
            Field('source', 'string'),
            migrate=False  # Don't migrate if table exists
        )
        db.commit()
