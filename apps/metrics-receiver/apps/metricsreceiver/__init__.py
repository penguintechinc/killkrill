...
fleet_metrics_counter = Counter(
    "killkrill_fleet_metrics_received_total", "Fleet metrics received", ["host"]
)
health_checks = Counter(
    "killkrill_metrics_receiver_health_checks_total", "Health checks", ["status"]
)
...
def ingest_fleet_metrics():
    ...
    try:
        ...
        processed_count = 0
        host_identifier = data.get(
            "hostIdentifier", data.get("host_identifier", "unknown")
        )
        ...
        for metric_data in metrics_data:
            try:
                ...
                metric_name = metric_data.get(
                    "name", metric_data.get("metric_name", "unknown")
                )
                metric_value = metric_data.get(
                    "value", metric_data.get("metric_value", "0")
                )
                ...
                db.received_metrics.insert(
                    ...
                    metric_value=(
                        float(metric_value)
                        if str(metric_value).replace(".", "").isdigit()
                        else 0.0
                    ),
                    labels=json.dumps(labels_data),
                    timestamp=datetime.utcnow(),
